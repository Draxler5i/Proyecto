...
const createUser = async (request: Express.Request, response: Express.Response) => {
    ...
    if (!validate.validateEmail(email)) return response.status(400).send(`Not valid email. `)
    if (age < 18) return response.status(400).send(`Age not valid, must be greater than 18. 
    We do not allow minors to make purchases, as one of our intern politics.`)
    if (!validate.validatePass(password)) return response.status(400).send(`Password must be at least of 6 characters and maximum 12 characters. 
    ...
    try {
        ...
        await client.query(`INSERT INTO public.user(name, last_name, email, password, age, nit) VALUES ($1, $2, $3, $4, $5, $6)`,
            [name, last_name, email, password_encrypted, age, nit],
            async (error: Error, results: typeof QueryResult) => {
                if (error) {
                    response.status(400).send(`A problem occured when trying to add new user`)
                }
                await client.query(`INSERT INTO creditCard(card_number, card_name, expiration, cvv, user_id, saldo) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
                    ...
                    (error: Error, res: typeof QueryResult) => {
                        if (error) {
                            response.status(400).send(`A problem occured when trying to add user card`)
                        }
...
const updateUser = async (request: Express.Request, response: Express.Response) => {
    ...
    try {
        ...
        await client.query(`UPDATE public.user SET email = $1, password = $2, age = $3, nit=$4  WHERE user_id = $5`,
            [email, new_password_encrypted, age, nit, id], async (error: Error, _results: typeof QueryResult) => {
                if (error) {
                    throw error
                }
                await client.query(`UPDATE public.creditcard SET card_number = $1, card_name = $2, expiration = $3, cvv=$4, saldo =$5 WHERE user_id = $6`,
                    [card_number, card_name, expiration, cvv, saldo, id], (error: Error, _results: typeof QueryResult) => {
                        if (error) {
                            throw error
                        }
