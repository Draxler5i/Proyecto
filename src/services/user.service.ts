...
const createUser = (request: Express.Request, response: Express.Response) => {
    ...
    if (age >= 18) return response.send('Age not valid, must be greater than 18. We do not allow minors to make purchases, as one of our intern politics.')
    if (validate.validatePass(password)) return response.send('Password must be at least of 6 characters and maximum 12 characters. Password must contain at least: a number, an uppercase letter, a lowercase letter, a digit, a special character and NO blanck spaces')
    try {
        client.query('INSERT INTO public.user(name, last_name, email, password, age, nit) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [name, last_name, email, encrypt.encryptWithAES(password), age, nit], (error: Error, results: typeof QueryResult) => {
            if (error) {
                throw error
            }
