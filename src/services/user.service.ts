...
const createUser = async (req: Express.Request, res: Express.Response) => {
    ...
    try {
        ...
        const card = await client.query(`INSERT INTO creditCard(card_number, card_name, expiration, cvv, user_id, saldo) 
                VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
            [card_number, card_name, expiration, cvv, user.rows[0].user_id, saldo])
        res.status(201).send(`User added with user name: ${user.rows[0].name}, lastname: ${user.rows[0].last_name}, email: ${user.rows[0].email}, age: ${user.rows[0].age}, nit: ${user.rows[0].nit}, card name: ${card.rows[0].card_name}, expiration: ${card.rows[0].expiration}`)
...
const deleteUser = async (req: Express.Request, res: Express.Response) => {
    ...
    try {
        ...
        const sells = await client.query(`select * from public.sells WHERE user_id = $1`, [id])
        for (let index = 0; index < sells.rowCount; index++) {
            await client.query(`DELETE FROM public.detail WHERE sells_id = $1`, [sells.rows[index].sells_id])
            await client.query(`DELETE FROM public.sells WHERE sells_id = $1`, [sells.rows[index].sells_id])
        }
