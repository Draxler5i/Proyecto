...
import encryptor from '../../src/security/encryp'
...
describe('Auth Controller, verify http methods', () => {
	...
	it("It should return a new user created", async () => {
		try {
			...
			when(validateUserMock.isValid(PAYLOAD_REQ.user.email)).thenResolve(true);
			when(validateCardMock.isValid(PAYLOAD_REQ.card)).thenResolve(true);
			when(userServiceMock.existUser(PAYLOAD_REQ.user.email)).thenResolve(USER_ROWS);
			when(encryptorMock.encrypt(PAYLOAD_REQ.user.password)).thenResolve(PASSWORD_ENCRIPTED);
			when(userServiceMock.postUser(PAYLOAD_REQ.user, PAYLOAD_REQ.card)).thenResolve(USER_SAVED_DB);

			const response = await request(app).post('/api/users').send(PAYLOAD_REQ);
